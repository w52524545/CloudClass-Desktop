      ...
      <Popover content={<SignalContent />} placement="bottomLeft">
        <div className="fcr-biz-header-title-wrap">
          <div
            ...
            className={`biz-signal-quality ${networkQualityClass}`}>
            <SvgImg
          ...
          <div style={{ display: 'inline-block' }}>浙二会诊平台</div>
        </div>
      </Popover>
    );
  }),
...
const StartButton = observer(() => {
  const { navigationBarUIStore } = useStore();
  const { isBeforeClass, IsOnClass, startClass } = navigationBarUIStore;
  return isBeforeClass ? (
    <Button size="xs" onClick={() => startClass()}>
      {IsOnClass ? '会议结束' : transI18n('begin_class')}
    </Button>
  ) : null;
});
